...
void test_speedup_100k_merge_path() {
  ...
  const double parallel_s = bench_best(
      [](std::vector<int>& v) { merge_sort::sort(v); }, input, kSpeedupWarmup, kSpeedupTimedRuns);
...
void test_speedup_1m_rank() {
  ...
  const double parallel_s = bench_best(
      [](std::vector<int>& v) { merge_sort::sort_by_rank(v); }, input, kSpeedupWarmup,
      kSpeedupTimedRuns);
...
int main() {
  ...
  run_test("20 elements", [&] { test_20_elements(sort_parallel); });
  run_test("10000 random elements", [&] { test_random(10'000, 42, sort_parallel); });
  run_test("100000 random elements", [&] { test_random(100'000, 12345, sort_parallel); });

  const auto sort_by_rank = [](std::vector<int>& v) { merge_sort::sort_by_rank(v); };

  std::cout << kDim << "merge_sort tests (parallel, splitters-by-rank)" << kReset << '\n';
  run_test("rank: 10 elements", [&] { test_10_elements(sort_by_rank); });
  run_test("rank: 20 elements", [&] { test_20_elements(sort_by_rank); });
  run_test("rank: 1000 random elements", [&] { test_random(1'000, 88, sort_by_rank); });
  run_test("rank: 10000 random elements", [&] { test_random(10'000, 89, sort_by_rank); });
  run_test("rank: 100000 random elements", [&] { test_random(100'000, 90, sort_by_rank); });
